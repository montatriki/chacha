...
export function CardMedia({ items, className, drift = false, onLabel }: { items: Media[]; className?: string; drift?: boolean; onLabel?: (label: string | undefined) => void }) {
  ...
  const render = (m: Media, key: string, fading: boolean) =>
    m.type === "video" ? (
      <video key={key} ref={fading ? undefined : videoRef} src={m.src} poster={m.poster} className={`absolute inset-0 h-full w-full object-cover ${className ?? ""}`} muted playsInline autoPlay loop={items.length < 2} preload="auto" onCanPlay={(e) => { e.currentTarget.play().catch(() => {}); }} style={{ opacity: fading ? 0 : 1, transition: "opacity 900ms ease" }} />
    ) : (
